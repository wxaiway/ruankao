...
import { CaseAnalysis, AnswerDisplayMode } from '@/lib/answer-questions';

interface PageState {
  questions: CaseAnalysis[];
  displayMaps: any;
  loading: boolean;
  error: string | null;
  currentIndex: number;
...
export default function CaseAnalysisChapterPage() {
    ...
    async function loadChapterData() {
      try {
        ...
        const [questions, displayMaps] = await Promise.all([
          answerQuestionService.getCaseAnalysisByChapter(chapter),
          answerQuestionService.getCaseAnalysisDisplayMaps()
        ]);
        ...
        <div className="text-center max-w-md mx-auto p-6">
          ...
          <div className="space-y-3">
            <button
              ...
              className="w-full btn bg-blue-600 hover:bg-blue-700 text-white px-4 py-2"
            >
              重新加载
            </button>
            <button
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-50 dark:bg-gray-900">
      ...
      {state.navigation && (
        <QuestionNavigation
          type="case-analysis"
          title={currentQuestion?.title || '案例分析'}
